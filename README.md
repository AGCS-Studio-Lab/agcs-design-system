...
> **v5.6 (2026-09-25): the deck grid and the app grid share one format.** The step goes from 40px to **48px at 4K**: a 3840 slide is always seen scaled, and at 1280 it shrinks to a third, so 48px reads as 16px and the 3px line as 1px -- exactly the agcs-ui grid. 48 divides both canvas dimensions (80 × 45 cells). What had to land on a line moved with the step: the edge padding goes from 160px to **144px** (3 cells), the divider header region from 640px to **624px** (13 cells; the content one stays at 480px, 10 cells), chart bars in **whole 48px cells**, and the System diagram is redrawn on the cells. The line stays at 3px, and the same day it went **lighter**: black at **6%** on Paper and white at **8%** on Obsidian (it was 9% / 11%). At 1280 that is a 1px line at ~1.14:1, still visible on a screen; check a projector before going lower. `tools/fix-pptx.py` rebuilds the PowerPoint grid at 48px and the new alpha. On the same day the cover icon grows to **1776px** and the divider icon to **1056px**.
>
