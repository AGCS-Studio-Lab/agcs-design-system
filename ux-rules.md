# UX & Accesibilidad — capa de producto AGCS (v3)
...
(`SKILL.md` / `colors_and_type.css` de este repo): donde una regla UX genérica
choque con la marca, **manda la marca**. Este doc ya tiene esas adaptaciones
hechas.
...
- **Exactamente un elemento lime por pantalla — ni cero ni dos** (acción primaria / item activo / un dato). Es un mínimo antes que un máximo: **una pantalla sin lime es un defecto**, igual que una con dos. Si nada merece el acento, lo lleva el icono temático en `verde1/`.
...
- **Color en gráficos**: series en escala de grises; **lime solo para resaltar UNA serie o el dato clave**; los colores funcionales v3 (data/warn/risk) solo cuando el dato ES ese rol (p.ej. salud de una initiative), exactos y con label. Nunca paletas categóricas arbitrarias.
...
- **Error**: di **qué pasó, por qué y cómo resolverlo**, en español, conclusión primero, sin códigos ni stack. El peso lo da el copy; risk `#ED1C24` solo como marca funcional con label, jamás pantallas teñidas de rojo.
...
3. Antes de cerrar: `evaluate_design` (o secciones 1-4 como checklist). Verifica contraste real (lime y warn), foco por teclado y los tres estados. El build de la app debe pasar.
