...
1. **Color is restricted.** Core: Obsidian `#000000`, Paper `#FFFFFF`, **Lime `#BEFF3A`** (must equal the icon library, which ships as baked PNGs at this value), Carbon `#1A1A1A`, Mist `#E5E5E5`. **v3 functional palette (2026-07-20), role-bound and exact:** Data blue `#00A1F1` (data viz / system / information layers), Signal yellow `#FFFF00` (warnings / attention), Signal red `#ED1C24` (risk / critical / blockers). Functional colors are never decorative. **Any OTHER blue, green, amber, red, purple, or any gradient stays off-brand** -- decorative blues still put AGCS in the McKinsey / BCG / Deloitte cluster.
